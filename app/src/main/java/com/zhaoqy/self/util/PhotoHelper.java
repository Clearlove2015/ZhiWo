...
public class PhotoHelper {
	...
	public static String getCachePath() {
		return FileUtils.getFileStorePath();
	}

	public static String getPath() {
		String kuid = "username";
	...
	public static byte[] getUserPhotoData(String photoPath) {
		if (!TextUtils.isEmpty(photoPath)) {
			File file = new File(photoPath);
			byte[] photoData = FileUtils.fileToByte(file);
			return photoData;
		} else {
			Log.e("ols", "photoPath null or empty");
			return null;
		}
	}
	...
	public static void openCamra(Activity activity, String cutPath, String NN_path, int requestCode) {
		if (!TextUtils.isEmpty(cutPath)) {
			if (("ZTE N5S").equals(getMobileModel())) {
				PhotoHelper.takePhoto(activity, Uri.fromFile(new File(NN_path, "tmphoto.jpg")), requestCode);
			} else {
				PhotoHelper.takePhoto(activity, Uri.fromFile(new File(cutPath)), requestCode);
			}

		} else {
			Log.d(TAG, "未获取到保存图片的路径");
		}
	}
	 ...
	 * @return void
	 */
	public static void cropImageUri(Activity activity, Uri uri, int aspectX, int aspectY, int outputX, int outputY,
			int requestCode) {
	...
	public static void saveUserPhotoUrl(String webUrl) {
		...
		if (!TextUtils.isEmpty(webPath)) {
			try {
				String paramsPath = FileUtils.getFileStorePath();
